...
class VolatilityAdaptiveScalping(bt.Strategy):
    ...
    params = (
        ...
        ("rsi_oversold", 30),  # Oversold RSI threshold
        ("sma_period", 50),  # SMA calculation period
        ("min_atr", 50.0),  # Minimum ATR value
        ("max_atr", 200.0),  # Maximum ATR value
        ("max_drawdown", 0.4),  # Maximum drawdown
    )
    ...
    def __init__(self):
        ...
        # RSI indicator for momentum measurement
        self.rsi = bt.indicators.RSI(self.data, period=self.p.rsi_period)

        # SMA indicator for trend measurement
    ...
    def next(self):
        ...
        if not (self.p.min_atr <= current_atr <= self.p.max_atr):
            return
        
        # Check drawdown
        current_drawdown = (self.broker.getvalue() / self.initial_cash) - 1
...
def run_backtest(data, plot=False, verbose=True, optimize=False, **kwargs):
    ...
    cerebro.addstrategy(VolatilityAdaptiveScalping, **kwargs)
...
def evaluate(individual, data):
    ...
    try:
        ...
        max_dd = abs(results.get("Max. Drawdown [%]", 0))

        # Penalize strategies with too few trades or excessive drawdown
        if trades < 20 or max_dd > 30:
            return (-np.inf,)

        # Combine metrics into a single fitness score with drawdown consideration
        fitness = (
            (sharpe * 0.3) + 
            (win_rate * 0.3) + 
            (ret * 0.3) + 
