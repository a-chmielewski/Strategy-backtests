    ...
    def next(self):
        ...
        if not self.position:
            if current_close > self.vwap[0]:
                self.buy_signal_candles.append({'high': current_high, 'low': current_low, 'close': current_close})
                if len(self.buy_signal_candles) > 5:
                    self.buy_signal_candles.pop(0)
            elif current_close < self.vwap[0]:
                self.sell_signal_candles.append({'high': current_high, 'low': current_low, 'close': current_close})
