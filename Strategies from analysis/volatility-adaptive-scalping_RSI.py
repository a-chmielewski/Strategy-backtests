...
class VolatilityAdaptiveScalping(bt.Strategy):
    ...
    def __init__(self):
        ...
        # Initialize all position tracking variables to None or appropriate default values
        self.stop_loss = 0.0
        self.take_profit = 0.0
        self.trailing_stop = 0.0  # Initialize with 0.0 instead of None
    ...
    def next(self):
        ...
        # Long entry
        if (current_price > self.highest_high[-1]) and (current_rsi > self.p.rsi_overbought):
        ...
        # Short entry
        elif (current_price < self.lowest_low[-1]) and (current_rsi < self.p.rsi_oversold):
            self.stop_loss = current_price + (self.atr[0] * self.p.stop_loss_atr)
            self.take_profit = current_price - (self.atr[0] * self.p.take_profit_atr)
            self.trailing_stop = self.stop_loss
