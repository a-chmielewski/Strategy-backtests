...
class MomentumBreakoutStrategy(bt.Strategy):
    """Momentum-based breakout strategy using RSI and ATR for volatility-adjusted entries and exits"""
...
class StrategyDataCollector:
    ...
    def __init__(self):
        self.base_path = Path("strategy_database") / "Momentum_Breakout"
    ...
    def save_backtest_results(self, strategy_name, results, strategy_params, data_info, trades_data, initial_capital, leverage):
        ...
        strategy_data = {
            ...
            "strategy_description": "RSI-based momentum breakout strategy with volatility filters",
            "data_info": data_info,
            "parameters": {
        ...
        # Save to file
        filename = f"Momentum_Breakout_{data_info['symbol']}_{data_info['timeframe']}_{self.current_timestamp}.json"
...
def run_backtest(data, plot=False, verbose=True, **kwargs):
    ...
    try:
        ...
        sqn = calculate_sqn(trade_recorder)  # Pass the trades list directly
    except Exception as e:
