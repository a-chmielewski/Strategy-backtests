...
class StrategyDataCollector:
    ...
    def __init__(self, base_path="strategy_database/Double_EMA_StochOsc"):
    ...
    def save_backtest_results(self, strategy_name, results, strategy_params, data_info, trades_data, 
        ...
        strategy_data = {
            ...
            "strategy_description": "Double EMA with Stochastic Oscillator Strategy",
            "data_info": data_info,
            "parameters": {
                "ema_slow": strategy_params.get("ema_slow", 50),
                "ema_fast": strategy_params.get("ema_fast", 150),
                "stoch_k": strategy_params.get("stoch_k", 5),
                "stoch_d": strategy_params.get("stoch_d", 3),
                "slowing": strategy_params.get("slowing", 3),
        ...
        # Save to file
        filename = f"Double_EMA_StochOsc_{data_info['symbol']}_{data_info['timeframe']}_{self.current_timestamp}.json"
...
def run_backtest(data, plot=False, verbose=True, **kwargs):
    ...
    saved_path = collector.save_backtest_results(
        strategy_name="Double_EMA_StochOsc",
